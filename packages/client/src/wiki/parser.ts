...
export class WikiParser {
  private readonly prototypeRule: RegExp = new RegExp(
    /(?<name>[A-Za-z_0-9]+)\((?<args>.*)\) -> (?<returnType>Void|.+)/,
  );
  private readonly argumentRule: RegExp = new RegExp(
    /((?<flag>(const|out|opt) )?(?<name>[A-Za-z0-9_]+): ?[A-Za-z0-9_\\\[\]:]+,?)*/g,
  );

  private readonly hintRule: RegExp = new RegExp(
    /\{% hint style="(info|success|warning|danger)" %}\n.*\n\{% endhint %}/gm,
  );
  ...
  public parseClass(file: WikiFileDto, name: string): WikiClassDto {
    ...
    const headerDescription: Tokens.Heading | undefined = stream.nextHeading(
      2,
      true,
    );
    const comment: string = this.parseDescription(stream, headerDescription);
    const headerFunctions: Tokens.Heading | undefined = stream.nextHeading(
      2,
      true,
    );
  ...
  private parseFunctions(stream: WikiTokenStream): WikiFunctionDto[] {
    ...
    while (stream.hasNext()) {
      ...
      const method: WikiFunctionDto | undefined = this.parseFunction(
        stream,
        headerFunction,
      );
  ...
  private isHeader(text: string, token?: Token): boolean {
    ...
    return (
      token.type === "heading" &&
      token.depth === 2 &&
      token.text.trim().toLowerCase() === text.trim().toLowerCase()
    );
  }
  ...
  private computeHashFromPrototype(prototypeMatch: RegExpMatchArray): number {
    ...
    const args: string[] = [];
    const argsMatch: IterableIterator<RegExpExecArray> = prototypeMatch.groups![
      "args"
    ].matchAll(this.argumentRule);
    ...
    const returnType: RedTypeAst | undefined = RedTypeAst.fromPseudocode(
      prototypeMatch.groups!["returnType"],
    );
